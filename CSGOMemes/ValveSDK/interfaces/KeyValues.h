...
{
    static void KeyValues_SetString(KeyValues* key, const char* value)
    {
        static auto key_values_set_string = reinterpret_cast<void(__thiscall*)(void*, const char*)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC A1 ? ? ? ? 53 56 57 8B F9 8B 08 8B 01"));
        key_values_set_string(key, value);
    }

    static void GameFunc_InitKeyValues(KeyValues* key, const char* name)
    {
        static auto key_values = reinterpret_cast<void(__thiscall*)(void*, const char*)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC 51 33 C0 C7 45 ? ? ? ? ? 56 8B F1 81 26 ? ? ? ? C6 46 03 ? 89 46 10 89 46 18 89 46 14 89 46 1C 89 46 04 89 46 08 89 46 0C FF 15 ? ? ? ? 6A 01 FF 75 08 8D 4D FC 8B 10 51 8B C8 FF 52 24 8B 0E 33 4D FC 81 E1 ? ? ? ? 31 0E 88 46 03"));
        key_values(key, name);
    }

    static void KeyValues_SetUint64(KeyValues* key, const char* keyName, int value, int value2)
    {
        static auto key_values_set_uint64 = reinterpret_cast<void(__thiscall*)(void*, const char*, int, int)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC 56 6A 01 FF 75 08"));
        key_values_set_uint64(key, keyName, value, value2);
    }

    static const char* KeyValues_GetString(KeyValues* key, const char *keyName, const char *defaultValue)
    {
        static auto key_values_get_string = reinterpret_cast<const char*(__thiscall*)(void*, const char*, const char*)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC 83 E4 C0 81 EC ? ? ? ? 53 8B 5D 08"));
        return key_values_get_string(key, keyName, defaultValue);
    }

    static int KeyValues_GetInt(KeyValues* key, const char *keyName, int defaultValue)
    {
        static auto key_values_get_int = reinterpret_cast<int(__thiscall*)(void*, const char*, int)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC 6A ? FF 75 08 E8 ? ? ? ? 85 C0 74 45"));
        return key_values_get_int(key, keyName, defaultValue);
    }

    static KeyValues* KeyValues_FindKey(KeyValues* key, const char *keyName, bool bCreate = false)
    {
        static auto key_values_find_key = reinterpret_cast<KeyValues*(__thiscall*)(void*, const char*, bool)>(utils::pattern_scan(GetModuleHandleW(L"client.dll"), " 55 8B EC 83 EC 1C 53 8B D9 85 DB"));
        return key_values_find_key(key, keyName, bCreate);
    }

    static void KeyValues_SetInt(KeyValues* key, const char *keyName, int Value)
    {
...
public:
    PVOID operator new(size_t iAllocSize)
    {
        ...
        if (!plebian)
        {
            ...
            oKeyValuesSystem KeyValuesSystem = (oKeyValuesSystem)GetProcAddress(GetModuleHandleW(L"vstdlib.dll"), "KeyValuesSystem");

            plebian = KeyValuesSystem();
        }

        typedef PVOID(__thiscall* oAllocKeyValuesMemory)(PVOID, size_t);
        return CallVFunction<oAllocKeyValuesMemory>(plebian, 1)(plebian, iAllocSize);
    }
    ...
    void operator delete(PVOID pMem)
    {
        ...
        CallVFunction<oFreeKeyValuesMemory>(plebian, 2)(plebian, pMem);
    }

private:
    static void* KeyValuesSystem;
