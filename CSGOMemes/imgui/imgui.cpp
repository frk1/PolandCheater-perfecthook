...
ImGuiStyle::ImGuiStyle()
{
    ...
    Colors[ImGuiCol_TitleText] = ImVec4(0.90f, 0.90f, 0.90f, 1.00f);
    ...
    if(first_begin_of_the_frame) {
        ...
        if(window->Collapsed) {
            ...
            if(flags & ImGuiWindowFlags_ShowBorders && !(flags & ImGuiWindowFlags_GroupBox)) {
                ...
                if(text_size.x > 1.0f) {
                    window->DrawList->AddLine(window->Pos, window->Pos + ImVec2{ g.Style->WindowPadding.x * 0.5f, 0.0f }, GetColorU32(ImGuiCol_Border));
                    window->DrawList->AddLine(window->Pos + ImVec2{ g.Style->WindowPadding.x * 0.5f + text_size.x + g.Style->WindowPadding.x, 0.0f }, window->Pos + ImVec2{ window->Size.x, 0.0f }, GetColorU32(ImGuiCol_Border));
                    ...
                    window->DrawList->PushClipRectFullScreen();
                    RenderText(window->Pos + ImVec2{ g.Style->WindowPadding.x, -text_size.y / 2 }, name, NULL, true);
                    window->DrawList->PopClipRect();
                } else {
                    window->DrawList->AddRect(window->Pos + ImVec2(1, 1), window->Pos + window->Size + ImVec2(1, 1), GetColorU32(ImGuiCol_BorderShadow), window_rounding);
                    window->DrawList->AddRect(window->Pos, window->Pos + window->Size, GetColorU32(ImGuiCol_Border), window_rounding);
        ...
        if(!(flags & ImGuiWindowFlags_NoTitleBar)) {
            ...
            RenderTextClipped(text_min, text_max, name, NULL, &text_size, GetColorU32(ImGuiCol_TitleText), style.WindowTitleAlign, &clip_rect);
        }
...
struct ImGuiStyleVarInfo
{
    ImGuiDataType   Type;
    ImU32           Offset;
    void*           GetVarPtr() const { return (void*)((unsigned char*)&ImGui::GetStyle() + Offset); }
};
...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    RenderTextClipped(bb.Min + style.FramePadding, bb.Max - style.FramePadding, label, NULL, &label_size, GetColorU32(ImGuiCol_Text), style.ButtonTextAlign, &bb);
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label, const char* label_end)
{
    ...
    if(display_frame) {
        ...
        RenderTextClipped(text_pos, bb.Max, label, label_end, &label_size, GetColorU32(ImGuiCol_Text));
    } else {
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos + ImVec2{ label_size.x + style.ItemInnerSpacing.x, 0.0f }, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y*2.0f));
    ...
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL, GetColorU32(ImGuiCol_Text), ImVec2(0.5f, 0.5f));
...
bool ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    RenderTextClipped(ImVec2(frame_bb.Min.x, frame_bb.Min.y + style.FramePadding.y), frame_bb.Max, value_buf, value_buf_end, NULL, GetColorU32(ImGuiCol_Text), ImVec2(0.5f, 0.0f));
    if(label_size.x > 0.0f)
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    RenderTextClipped(bb.Min, bb_with_spacing.Max, label, NULL, &label_size, GetColorU32(ImGuiCol_Text), ImVec2(0.0f, 0.0f));
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
    ...
    ImVec2 size;
    size.x = 0.0f;
    size.y = GetTextLineHeightWithSpacing() * height_in_items_f + ImGui::GetStyle().ItemSpacing.y;
